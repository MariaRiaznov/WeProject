...
const RatingByVolunteer = () => {
    ...
    useEffect(() => {
        ...
        fetchUserId();
        setReceiverId(request.user_id);
    }, [request.user_id]);

    useEffect(() => {
        console.log('User ID:', userId);
        console.log('Receiver ID:', receiverId);
        console.log('Call ID:', request.call_id);
    }, [userId, receiverId, request.call_id]);
    ...
    const handleSendRating = async () => {
        if (!userId || !receiverId || !request.call_id) {
            console.error('User ID, Receiver ID, or Call ID is missing');
            Alert.alert('Error', 'User ID, Receiver ID, or Call ID is missing');
            return;
        }

        console.log('Sending Rating - User ID:', userId, 'Receiver ID:', receiverId, 'Call ID:', request.call_id);
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.infoText}>דרג את המשתמש אשר ביקש עזרה:</Text>
            <RatingBar rating={starRating} onRatingPress={handleStarRatingPress} />

            <Text style={[styles.infoText, styles.centerText]}>דרג את האופן שבו הקריאה באתר הייתה זהה לאירוע בפועל:</Text>
            <SimpleRating rating={simpleRating} onRatingChange={handleSimpleRatingChange} />

            <Text style={[styles.reportText, { color: 'red' }]}>דווח/י על המשתמש:</Text>
            <TextInput
                style={styles.reportInput}
                multiline
                numberOfLines={4}
                placeholder="כתוב כאן את הדיווח שלך"
                value={reportText}
...
export default RatingByVolunteer;
